...
function Login() {
  ...
  const handleGoogleSuccess = async (credentialResponse) => {
    ...
    try {
      ...
      if (data.success) {
        setSuccess('Login Google riuscito!');
        ...
        setError(data.message || 'Errore durante il login Google');
      }
    } catch (err) {
      setError('Errore di connessione');
    } finally {
  ...
  return (
    <LoginContainer>
      <LoginCard>
        ...
        <Title>Sign In to <span style={{background: `linear-gradient(90deg, ${COLORS.secondary} 0%, ${COLORS.primary} 100%)`, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', fontWeight: 900}}>BOIOLAX</span></Title>
        ...
        <RegisterLink>
          Non hai un account? <StyledLink to="/register">Registrati</StyledLink>
        </RegisterLink>
      </LoginCard>
    </LoginContainer>
  );
